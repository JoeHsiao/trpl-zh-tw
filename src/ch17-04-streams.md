...
Rust 中迭代器和 stream 的這種相似性意味著，我們實際上可以從任意迭代器建立一個 stream。和使用迭代器一樣，我們也可以透過呼叫 stream 的 `next` 方法，再等待其輸出，來處理它，如示例 17-21 所示。不過這段程式碼暫時還編譯不過。
...
遺憾的是，當我們嘗試執行這段程式碼時，編譯器並不會透過，而是報告說沒有可用的 `next` 方法：
...
[17-02-messages]: ch17-02-concurrency-with-async.html#透過訊息傳遞在兩個任務之間傳送資料
