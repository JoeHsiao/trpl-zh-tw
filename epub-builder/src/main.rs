...
fn resolve_includes(content: &str, current_file_path: &Path) -> String {
    ...
    let result = re.replace_all(content, |caps: &Captures| {
        ...
        if !included_abs_path.exists() {
            eprintln!("警告：引入檔案不存在 {:?} (在 {:?} 中引用)", included_abs_path, current_file_path);
            return format!("<!-- 缺失引入檔案 {} -->", file_spec);
        }
        
        let included_content = match std::fs::read_to_string(&included_abs_path) {
            Ok(c) => c,
            Err(e) => {
                eprintln!("讀取檔案失敗 {:?}: {}", included_abs_path, e);
                return format!("<!-- 讀取檔案失敗 {} -->", file_spec);
            }
        };
...
fn main() {
    ...
    if !summary_path.exists() {
        eprintln!("錯誤：未找到 src/SUMMARY.md，請在專案根目錄下執行此程式。");
        std::process::exit(1);
    }
    
    let summary_content = std::fs::read_to_string(&summary_path).expect("讀取 SUMMARY.md 失敗");
    ...
    println!("成功解析 SUMMARY.md，共找到 {} 個章節檔案。", file_list.len());
    ...
    for file_path_str in &file_list {
        ...
        if !full_path.exists() {
            eprintln!("警告：檔案 {:?} 不存在，已跳過。", full_path);
            continue;
        }
        
        let content = std::fs::read_to_string(&full_path).unwrap_or_else(|_| panic!("讀取檔案失敗：{:?}", full_path));
        ...
        let file_id = file_id_map.get(&filename).expect("找不到對應的 file_id").clone();
    ...
    println!("已生成合並後的臨時 Markdown 檔案。");
    
    println!("正在呼叫 Pandoc 編譯 EPUB...");
    ...
    match status {
        Ok(s) if s.success() => {
            println!("恭喜！編譯成功，生成檔案：{}", output_epub);
        }
        Ok(s) => {
            eprintln!("編譯失敗：pandoc 退出狀態為 {}", s);
        }
        Err(e) => {
            if e.kind() == std::io::ErrorKind::NotFound {
                eprintln!("錯誤：系統未檢測到 'pandoc' 命令，請確認 Pandoc 已正確安裝並加入系統環境變數 PATH。");
            } else {
                eprintln!("編譯失敗：{}", e);
            }
        }
