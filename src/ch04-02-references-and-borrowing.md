...
變數 `s` 的有效作用域與其他函式引數相同，不過當 `s` 停止使用時，它所指向的值不會被丟棄，因為 `s` 並不擁有它。當函式把引用而不是實際值作為引數時，就不需要透過返回值來交還所有權，因為函式從未擁有過它。
...
正如變數預設是不可變的，引用預設也是不可變的。我們不允許透過引用修改它指向的值。

### 可變引用

我們透過一個小調整就能修復示例 4-6 程式碼中的錯誤，允許我們修改一個借用的值，這就是 **可變引用**（*mutable reference*）：
...
資料競爭會導致未定義行為，難以在執行時追蹤，並且難以診斷和修復；Rust 透過拒絕編譯存在資料競爭的程式碼來避免此問題！
...
讓我們嘗試建立一個懸垂引用，看看 Rust 如何透過一個編譯時錯誤來防止它：
