...
這裡列出的 trait 是僅有的在標準庫中定義且能通過 `derive` 在型別上實現。標準庫中定義的其它 trait 不能通過 `derive` 在型別上實現。這些 trait 不存在有意義的預設行為，所以由你負責以合理的方式實現它們。
...
`Debug` trait 用於開啟格式化字串中的除錯格式，其通過在 `{}` 佔位符中增加 `:?` 表明。
...
`Copy` trait 允許你通過只複製儲存在棧上的位來復制值；無需執行額外的程式碼。查閱第四章 [“只在棧上的資料：複製”][stack-only-data-copy] 的部分來獲取有關 `Copy` 的更多資訊。
...
任何使用 `Copy` 的程式碼都可以通過 `Clone` 實現，但程式碼可能會稍慢，或者不得不在程式碼中的許多位置上使用 `clone`。
